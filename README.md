...
![sml-obj wireframe demo](assets/wireframe.png)

*Generated by [`examples/wireframe.sml`](examples/wireframe.sml) (`make
example`): a torus emitted as OBJ text, parsed with `Mesh.parseObj`, projected
through an `sml-glm` model/view/projection pipeline, and drawn as a depth-shaded
wireframe with the vendored `sml-raster`.*
